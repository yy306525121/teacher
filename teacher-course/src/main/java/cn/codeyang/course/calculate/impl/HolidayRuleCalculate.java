...
public class HolidayRuleCalculate implements CourseFeeCalculate {
    ...
    public List<CourseFee> calculate(LocalDate startDate, LocalDate endDate, List<CourseFee> courseFeeList) {
        List<HolidayRule> list = holidayRuleService.getListByDate(startDate, endDate);
            ...
            while (!tmpDate.isAfter(ruleEndDate)) {
                for (int i = firstTimeSlot.getSortInDay(); i <= lastTimeSlot.getSortInDay(); i++) {
                    ...
                    for (ClassInfo topClassInfo : ruleClassInfoList) {
                        ...
                        // 先判断是否存在
                        Optional<IgnoreItemDto> first = ignoreCoursePlanList.stream()
                                .filter(
                                        item -> item.getDate().equals(finalTmpDate) &&
                                                item.getTimeSlot().getId().equals(timeSlot.getId()) &&
                                                item.getClassInfo().getId().equals(topClassInfo.getId()))
                                .findFirst();
                        // 如果已经存在，不处理
                        if (first.isEmpty()) {
